...
df_mm2a <- clean_df('maternal_mortality_race_11_17')
df_mm2b <- clean_df('maternal_mortality_race_18_19') %>%
  rename(Race = Single.Race.6, Race.Code = Single.Race.6.Code)
df_mm_covid <- clean_df('maternal_mortality_race_20_22') %>%
  rename(Race = Single.Race.6, Race.Code = Single.Race.6.Code)

df_mmno1 <- clean_df('maternal_mortality_spec_race_00_10')
df_mmno2a <- clean_df('maternal_mortality_spec_race_11_17')
df_mmno2b <- clean_df('maternal_mortality_spec_race_18_19') %>%
  rename(Race = Single.Race.6, Race.Code = Single.Race.6.Code)
df_mmno_covid <- clean_df('maternal_mortality_spec_race_20_22') %>%
  rename(Race = Single.Race.6, Race.Code = Single.Race.6.Code)

df_mmot1 <- clean_df('other_mat_mortality_race_00_10')
df_mmot2a <- clean_df('other_mat_mortality_race_11_17')
df_mmot2b <- clean_df('other_mat_mortality_race_18_19') %>%
  rename(Race = Single.Race.6, Race.Code = Single.Race.6.Code)
df_mmot_covid <- clean_df('other_mat_mortality_race_20_22') %>%
  rename(Race = Single.Race.6, Race.Code = Single.Race.6.Code)

df_pr1 <- clean_df('pregrel_mortality_race_00_10')
df_pr2a <- clean_df('pregrel_mortality_race_11_17')
df_pr2b <- clean_df('pregrel_mortality_race_18_19') %>%
  rename(Race = Single.Race.6, Race.Code = Single.Race.6.Code)
df_pr_covid <- clean_df('pregrel_mortality_race_20_22') %>%
  rename(Race = Single.Race.6, Race.Code = Single.Race.6.Code)
...
mrg_births <- function(df_mort, df_nat) {
  ...
  df_mort[(df_mort$Race == 'Native Hawaiian or Other Pacific Islander') | 
            (df_mort$Race == 'Asian'), 'Race'] = 'Asian or Pacific Islander'
...
df_mm1 <- mrg_births(df_mm1, df_nat_race_01_10)
df_mm2 <- mrg_births(df_mm2, df_nat_race_11_19)
df_mm_covid <- mrg_births(df_mm_covid, df_nat_race_20_22)

df_mmno1 <- mrg_births(df_mmno1, df_nat_race_01_10)
df_mmno2 <- mrg_births(df_mmno2, df_nat_race_11_19)
df_mmno_covid <- mrg_births(df_mmno_covid, df_nat_race_20_22)

df_mmot1 <- mrg_births(df_mmot1, df_nat_race_01_10)
df_mmot2 <- mrg_births(df_mmot2, df_nat_race_11_19)
df_mmot_covid <- mrg_births(df_mmot_covid, df_nat_race_20_22)

df_pr1 <- mrg_births(df_pr1, df_nat_race_01_10)
df_pr2 <- mrg_births(df_pr2, df_nat_race_11_19)
df_pr_covid <- mrg_births(df_pr_covid, df_nat_race_20_22)
...
byars_conf_interval <- function(race, x, n, mult=100000, alpha=0.05) {
  O <- x
  z <- qnorm(1 - alpha/2)
...
df_ci_00_10 <- distinct(byars_conf_interval(df_crude_long$Mother.s.Race, df_crude_long$Deaths.x, df_crude_long$Births.x, 100000))
df_ci_00_10$time_period <- 'Rate.01.10'

df_ci_11_19 <- distinct(byars_conf_interval(df_crude_long$Mother.s.Race, df_crude_long$Deaths.y, df_crude_long$Births.y, 100000))
...
df_ci_covid <- distinct(byars_conf_interval(df_crude_covid_long$Mother.s.Race, df_crude_covid_long$Deaths.y, df_crude_covid_long$Births.y, 100000))
...
#   coord_flip()
# ggsave("figs/plt_census_pct_chg_mat_race_crude.png")
# 
# df_crude_covid %>%
#   ggplot(aes(x=Mother.s.Race, y=Pct.Change, fill=Type)) +
#   geom_bar(stat="identity", position=position_dodge()) +
...
#   coord_flip()
# ggsave("figs/plt_census_pct_chg_mat_race_crude_covid.png")
