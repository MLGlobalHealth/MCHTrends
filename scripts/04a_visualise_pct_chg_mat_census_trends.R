...
byars_conf_interval <- function(census, x, n, mult=100000, alpha=0.05) {
  O <- x
  z <- qnorm(1 - alpha/2)
...
df_ci_00_10 <- distinct(byars_conf_interval(df_crude_long$Census.Region, df_crude_long$Deaths.x, df_crude_long$Births.x, 100000))
df_ci_00_10$time_period <- 'Rate.01.10'

df_ci_11_19 <- distinct(byars_conf_interval(df_crude_long$Census.Region, df_crude_long$Deaths.y, df_crude_long$Births.y, 100000))
...
df_ci_covid <- distinct(byars_conf_interval(df_crude_covid_long$Census.Region, df_crude_covid_long$Deaths.y, df_crude_covid_long$Births.y, 100000))
...
ggplot(df_crude_all, aes(x = Census.Region, y = value, fill = Period)) +
  ...
  theme_minimal() 
ggsave("figs/plt_pct_chg_cause_spec_mat_census_crude.png")
ggsave("figs/plt_pct_chg_cause_spec_mat_census_crude.svg")
